...
export default class ConfirmarScreen extends Component {
    ...
    constructor(props) {
        super(props);
        this.state = { dataCarrito: [], total: '', isLoading: true, direccion: [], direcciones: [], usuario: [],envio:0 };
    }
    ...
    async componentDidMount() {
        try {
            ...
            this.setState({ dataCarrito: JSON.parse(response), direccion: JSON.parse(response_defa), direcciones: JSON.parse(response_direcc), usuario: JSON.parse(usuario), envio:parseFloat(envio) });
            this.setState({ total: this.state.dataCarrito.reduce((sum, value) => (typeof value.subtotal == "number" ? sum + value.subtotal : sum), 0) + this.state.envio, isLoading: false })
            //console.log()
        } catch (e) {
            console.log(e)
        }
    ...
    async confirmar (){
        ...
        const settings = {
            ...
            body: JSON.stringify({
                ...
                sinenvio: this.state.total-this.state.envio,
                createAt: tiempo
            })
        };
        try {
            const fetchResponse = await fetch(GLOBALS.BASE_URL + 'api/api.php?type=insertar&que=pedido', settings);
