...
import {
    Text, View, ActivityIndicator, TouchableOpacity, Alert, Dimensions, Image, StyleSheet, ScrollView
} from 'react-native'
...
export default class CarritoScreen extends Component {
    ...
    constructor(props) {
        super(props);
        this.state = { dataCarrito: [], total: '', subtotal: '', isLoading: true, nombre: '', isempty: true, info: [], direccion: [], direcciones: [], usuario: [], envio: 0 };
    }
    ...
    async componentDidMount() {
        try {
            ...
            const g_envio = await AsyncStorage.getItem("envio")
            const envio = JSON.parse(g_envio);

            const response_direcc = await AsyncStorage.getItem("direcciones")
            console.log(response)
            this.setState({ dataCarrito: JSON.parse(response), info: JSON.parse(response_inf), direccion: JSON.parse(response_defa), direcciones: JSON.parse(response_direcc), envio: parseFloat(envio) });
            console.log(this.state.dataCarrito)
            if (this.state.dataCarrito.length !== 0) {
                this.setState({ total: this.state.dataCarrito.reduce((sum, value) => (typeof value.subtotal == "number" ? sum + value.subtotal : sum), 0) + this.state.envio, subtotal: this.state.dataCarrito.reduce((sum, value) => (typeof value.subtotal == "number" ? sum + value.subtotal : sum), 0) })
                //console.log(this.state.dataCarrito.length)
                var link = GLOBALS.BASE_URL + 'api/api.php?type=consulta&que=nombre-establecimiento&id=' + this.state.dataCarrito[0].negocio;
    ...
    borrar(idprod, id) {
        ...
        this.setState({ isLoading: false, total: this.state.dataCarrito.reduce((sum, value) => (typeof value.subtotal == "number" ? sum + value.subtotal : sum), 0) + this.state.envio, subtotal: this.state.dataCarrito.reduce((sum, value) => (typeof value.subtotal == "number" ? sum + value.subtotal : sum), 0) })
        if (this.state.dataCarrito.length == 0) {
                ...
                <ScrollView>
                    ...
                    <View style={{ padding: 15 }}>
                        ...
                        <View style={{ flexDirection: 'row', width: '100%' }}>
                            ...
                            <Text style={{ textAlign: 'right', flex: 1, fontSize: 16, fontFamily: 'Oxygen-Regular' }}>${this.state.envio}.00</Text>
                        </View>
                        <View style={{ flexDirection: 'row', width: '100%' }}>
